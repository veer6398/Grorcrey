...
const ProductDetails = () => {
   ...
   const { category, id } = useParams();
    ...
    const [relatedProducts, setReletedProducts] = React.useState([]);
    ...
    useEffect(()=>{
        if(products.length > 0 && product){
            ...
            setReletedProducts(productsCopy.slice(0,5))
        }
    },[products])
        ...
        <div className="mt-12">
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                                <img key={i} className="w-3.5 md:w-4" src={i<4?assets.star_icon : assets.star_dull_icon} alt="" />
                        ))}
           ...
           <div className="flex flex-col items-center mt-20">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-6 mt-6 w-full">
                    {relatedProducts.filter((product)=>product.inStock).map((product, index)=>(<ProductCart key={index} product={product}/>))}
                </div>
